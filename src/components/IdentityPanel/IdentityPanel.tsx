...
import { PERSONA_STUBS, getPersonasBySpace } from '@/mocks/customerPersonas';
...
function buildTraits(profile: CustomerProfile): string[] {
  ...
  if (profile.projectProfile?.skillLevel && profile.projectProfile.skillLevel !== 'intermediate') {
    traits.push(`${profile.projectProfile.skillLevel.charAt(0).toUpperCase() + profile.projectProfile.skillLevel.slice(1)} skill`);
  }
  ...
  if (profile.projectProfile?.companyName) {
    traits.push(profile.projectProfile.companyName);
  }
  if (profile.projectProfile?.concerns?.length) {
    traits.push(profile.projectProfile.concerns[0].charAt(0).toUpperCase() + profile.projectProfile.concerns[0].slice(1));
  }
  return traits.slice(0, 5);
}
...
function renderProfileSections(customer: CustomerProfile) {
  ...
  const pp = customer.projectProfile;

  if (pp?.skillLevel || pp?.homeType || pp?.companyName) {
    sections.push(
      <Section key="project" title={customer.space === 'b2b' ? 'Business Profile' : 'Project Profile'} source="Contact" defaultOpen>
        <Field label="Skill Level" value={pp.skillLevel} />
        <Field label="Home Type" value={pp.homeType} />
        <Field label="Home Age" value={pp.homeAge} />
        <Field label="Company" value={pp.companyName} />
        <Field label="Company Type" value={pp.companyType} />
        <Field label="Trade" value={pp.tradeSpecialty?.join(', ')} />
        <Field label="License" value={pp.licenseNumber} />
        <Field label="Concerns" value={pp.concerns?.join(', ')} />
        <Field label="Preferred Brands" value={pp.preferredBrands?.join(', ')} />
      </Section>
    );
  }
        ...
        {customer.orders.map((o, i) => (
          <div key={i} className="py-1 border-b border-white/5 last:border-b-0">
            ...
            <div className="text-[10px] text-white/40">
              {o.lineItems.map(li => li.productName).join(', ')} &mdash; ${o.totalAmount.toLocaleString()}
            </div>
          </div>
        ))}
      </Section>
  ...
  if (customer.appendedProfile) {
    ...
    sections.push(
      <Section key="appended" title="Merkury Appended (3P)" source="Merkury">
        ...
        <Field label="Home Ownership" value={ap.homeOwnership} />
...
export const IdentityPanel: React.FC = () => {
  ...
  const { selectPersona, customer, selectedPersonaId, isResolving, isLoading, refreshProfile, resetPersonaSession, space, setSpace } = useCustomer();

  const stubs = PERSONA_STUBS.filter((s) => s.space === space);
  const activeStub = stubs.find((s) => s.id === selectedPersonaId) || PERSONA_STUBS.find((s) => s.id === selectedPersonaId);
  ...
  return (
    <div className="fixed top-4 right-4 z-50">
      {/* Space toggle + trigger button */}
      <div className="flex items-center gap-2">
        <div className="flex rounded-lg bg-black/60 backdrop-blur-sm border border-white/10 overflow-hidden">
          <button
            onClick={() => setSpace('consumer')}
            className={`px-2.5 py-1.5 text-[10px] font-medium transition-colors ${
              space === 'consumer' ? 'bg-brand-600 text-white' : 'text-white/50 hover:text-white/70'
            }`}
          >
            Consumer
          </button>
          <button
            onClick={() => setSpace('b2b')}
            className={`px-2.5 py-1.5 text-[10px] font-medium transition-colors ${
              space === 'b2b' ? 'bg-brand-600 text-white' : 'text-white/50 hover:text-white/70'
            }`}
          >
            B2B / Pro
          </button>
            ...
            >
                  ...
                  {customer && (
                    <div>
                      <div className="px-3 py-2 border-t border-white/10">
                        ...
                        {customer.email && (
                          <div className="text-[10px] text-white/50 mt-0.5">{customer.email} &mdash; {customer.id}</div>
                        )}
                      </div>
              ...
              <div className="p-3">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-[10px] text-white/30">{space === 'b2b' ? 'Pro personas' : 'Consumer personas'}</span>
                </div>
                <div className="space-y-1.5">
                  {stubs.filter((s) => s.id !== activeStub?.id).map((stub) => {
